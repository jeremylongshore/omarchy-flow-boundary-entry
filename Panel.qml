...
Panel {
 ...
 property string eventsJson: "[]"; property var rows: []; property bool loaded: false; property double nowMs: Date.now(); readonly property bool isAlert: false
 ...
 function open() { openedFromHotkey=false; root.controller.show(); root.refresh() } function openFromHotkey() { openedFromHotkey=true; root.controller.show(); root.refresh() } function close(){root.controller.hide()} function toggle(){if(root.opened)root.close();else root.openFromHotkey()} function switchPanel(d){return root.bar&&typeof root.bar.switchPanelFrom==="function"?root.bar.switchPanelFrom(root.barIdentity,d):false}
 function refresh(){nowMs=Date.now();rows=Model.parse(eventsJson,nowMs);loaded=true}
 function syncFromSettings(){var value=hostWidget&&hostWidget.settings?hostWidget.settings.eventsJson:undefined;if(typeof value==="string")eventsJson=value;refresh()}
 function mark(kind){var next=Model.appendEvent(eventsJson,kind,Date.now());eventsJson=next;if(bar&&bar.shell&&typeof bar.shell.updateEntryInline==="function"){var merged={};var current=hostWidget&&hostWidget.settings?hostWidget.settings:{};for(var key in current)if(key!=="id")merged[key]=current[key];merged.eventsJson=next;bar.shell.updateEntryInline(root.moduleName,merged)}refresh()}
 onHostWidgetChanged:syncFromSettings()
 Connections { target:root.hostWidget; function onSettingsChanged(){root.syncFromSettings()} }
  Timer { interval:30000; running:true; repeat:true; triggeredOnStart:true; onTriggered:root.refresh() }
 IpcHandler { target:root.ipcTarget; function open():void{root.openFromHotkey()} function close():void{root.close()} function show():void{root.openFromHotkey()} function hide():void{root.close()} function toggle():void{root.toggle()} function arrive():void{root.mark("arrive")} function leave():void{root.mark("leave")} function refresh():void{if(root.hostWidget&&typeof root.hostWidget.broadcast==="function")root.hostWidget.broadcast("refresh");else root.refresh()} }
 KeyboardPanel { id:panel; anchorItem:root.anchorItem; owner:root.barIdentity; bar:root.bar; open:root.opened; centerOnBar:true; focusTarget:keys; contentWidth:panel.fittedContentWidth(Style.space(520)); contentHeight:panel.fittedContentHeight(content.implicitHeight)
