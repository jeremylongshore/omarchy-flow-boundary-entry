...
test("marketplace preview is a focused 16:9 live-render composition", () => {
  ...
  assert.match(rig, /rawShellLogSha256/)
  assert.match(rig, /-path '\.\/e2e\/\*'/)
  assert.match(rig, /visualInspection:\{status:"pending"/)
  assert.match(rig, /BatchMode=yes/)
  assert.match(rig, /refusing to write a clean receipt for a warning-bearing shell log/)
  assert.match(rig, /--exclude=\.rig-proof\.json --exclude=\.render-proof\.json/)
  assert.match(rig, /--exclude=preview\.png/)
